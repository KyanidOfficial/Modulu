import { AltComparisonVector, AltLikelihood, UserRiskState } from './types';

function manhattanDistance(a: UserRiskState, b: AltComparisonVector): number {
  return (
    Math.abs(a.avgMessageLength - b.avgMessageLength) +
...
export function calculateAltLikelihood(current: UserRiskState, vectors: AltComparisonVector[], threshold: number): AltLikelihood {
  if (vectors.length === 0) {
