...
const banService = require("../../../services/moderation/ban.service")
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await banService.run({ guild: interaction.guild, member, moderatorId: interaction.user.id, reason })

    await safeReply(interaction, {
      embeds: [moderation({ action: "ban", target: `<@${member.id}>`, reason, moderator: `<@${interaction.user.id}>` })]
    })
  }
}
